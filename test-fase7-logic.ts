...
const mockAccounts = [{ id: "demo-user", name: "David Smith", role: "user" }];

const mockStaff = [{ id: "s1", name: "Nanami Putri", role: "owner" }];
...
async function runFinalTests() {
  ...
  const dbData = await fetchCmsFromDb();
  const localState = { cms: JSON.parse(JSON.stringify(dbData)) };
  ...
  const refreshedDbData = await fetchCmsFromDb();
  ...
  const finalDbData = await fetchCmsFromDb();
  ...
  console.log(
    "StaffPanel rows (Staff list):",
    staffRows.map((s) => s.name),
  );
  console.log(
    "CustomerPanel rows (Filtered Accounts):",
    customerRows.map((c) => c.name),
  );
